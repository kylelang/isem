...
\begin{frame}{Model Continuum}
  ...
  We can orient the possible models on a single continuum.
  ...
  \begin{enumerate}
    ...
    \item \emph{Relative/Incremental/Comparative Fit}
      \begin{itemize}
        ...
        \item Compare our model to the \emph{indepenence model}.
      \end{itemize}
  \end{enumerate}
...
\begin{frame}{SRMR}
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      Basically, the SRMR represents the average magnitude of the standadized model residuals.
    \end{column}
    \begin{column}{0.3\textwidth}
      \begin{align*}
