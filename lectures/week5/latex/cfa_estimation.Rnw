...
\begin{frame}{Optimization: Objective Function}
  ...
  In CFA, we minimize the \emph{model deviance}.
  \begin{align*}
    D_M &= -2 \left[ \ell \left( \hat{\Sigma}, \hat{\mu} \right) - \ell \left( \mathbf{S}, \bar{\mathbf{Y}} \right) \right]
  \end{align*}

  Where, $\ell ( \cdot )$ denotes a \emph{loglikelihood} function.
  \begin{itemize}
...
\begin{frame}{Optimization: Loglikelihood Functions}
  ...
  A loglikelihood function is a model-based distance metric.

  \begin{itemize}
    \item Derived from the assumed data distribution (i.e., multivariate normal, for the basic CFA)
  ...
  Say we fit two model, $M_a$ and $M_b$, to the same dataset. If $M_a$'s loglikelihood is closer to zero, we know that
  $M_a$ represents the data better than $M_b$ does.
  \begin{itemize}
...
\begin{frame}{Model Fit: Intuition}
  ...
  We can use \emph{model fit statistics/indices} to evaluate the second and third criteria.
...
\begin{frame}{$\chi^2$ Statistic}
  ...
  The $\chi^2$ statistic is the most fundamental fit statistic.
  \begin{itemize}
    \item Derived directly from the objective function used to estimate the model.
...
\begin{frame}{Problems with $\chi^2$}
  ...
  The $\chi^2$ test is a perfectly valid statistical test, and it does it's job very well, but we don't really care.

  \begin{align*}
    H_0: \hat{\Sigma} = \mathbf{S}\\[8pt]
    H_0: \hat{\Sigma} \neq \mathbf{S}
  \end{align*}
...
\begin{frame}{Different Ways to Quantify Fit}
  ...
  When evaluate model fit, we need to consider three models.
...
<<>>=
x <- runif(10, 0, 1)
y <- 0.5 + x + 0.25 * x^2 + rnorm(10)

data.frame(X = x, Y = y) |>
  ggplot(aes(X, Y)) +
  geom_point() +
  geom_smooth(method = "lm", formula = "~ x + I(x^2)")
@
