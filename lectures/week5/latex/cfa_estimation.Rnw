  ...
  \begin{enumerate}
    ...
    \item The \emph{independence model}
      \begin{itemize}
        ...
        \item All item associations are fixed to zero
      \end{itemize}
      \vc
    \item The \emph{saturated model}
      \begin{itemize}
        \item The "best" possible model
        \item All possible associations are estimated
  ...
  \begin{enumerate}
    \item \emph{Absolute Fit}
      \begin{itemize}
        ...
        \item How close is our model to the best possible model?
        \item Compare our model to the \emph{saturated model}.
      \end{itemize}
      \vc
    \item \emph{Relative/Incremental/Comparative Fit}
      \begin{itemize}
        \item How much better does our model perform than some poorer baseline model?
        \item How much improvement do we achieve over the worst possible model?
...
\begin{frame}{Problems with $\chi^2$}
  ...
  \begin{itemize}
    ...
    \item With a large enough sample, the null hypothesis must be reject for any model other than the exact data
      generating model.
...
\begin{frame}{RMSEA}
  ...
  The RMSEA is an \emph{absolute fit index} that evaluates deviations from \emph{close fit}.

  \begin{align*}
    \textrm{RMSEA} = \sqrt{\textrm{max} \left[ \frac{\chi^2 - df}{N \times df}, ~ 0 \right]}
  \end{align*}
...
\begin{frame}{CFI}
  ...
  \end{align*}
  Assuming the ordinary situation where $\chi^2 \geq df$, and the baseline model fits worse than our target model, we
  can simplify the CFI formula.
  \begin{align*}
...
\begin{frame}[fragile]{Example: Saturated Model}
...
\sectionslide{Model Complexity}

%----------------------------------------------------------------------------------------------------------------------%

\begin{frame}{DF \& Complexity}
