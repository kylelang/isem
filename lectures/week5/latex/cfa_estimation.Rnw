...
\begin{frame}{Problems with $\chi^2$}
  ...
  The $\chi^2$ test is a perfectly valid statistical test, and it does it's job very well, but we usually don't care.
  \begin{align*}
    H_0: \hat{\Sigma} = \mathbf{S}\\
    H_1: \hat{\Sigma} \neq \mathbf{S}
  \end{align*}
  This isn't a very interesting or useful hypothesis to test.
  \begin{itemize}
    \item With reasonably sized samples, the $\chi^2$ will almost always reject the null hypothesis.
    \item We wouldn't want a model that exactly replicated the observed sufficient statistics.
  \end{itemize}

  \vb

  In practice, we won't use the $\chi^2$ test to directly evaluate model fit. We'll use other fit indices derived from
  the $\chi^2$.
...
\begin{frame}{Different Ways to Quantify Fit}
  ...
  When evaluating model fit, we need to consider three models.
  \vc
  \begin{enumerate}
