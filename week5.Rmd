...
source(here::here("code", "chapter_setup.R"))
partDir <- paste0(partDir0, "week5/")
```

This week, we will continue with CFA. In particular, we'll be focusing on model estimation and model fit.

**Homework before the lecture**

1. Watch the [Lecture Recording](lecture-4.html) for this week.
1. Complete the [Reading](reading-4.html) for this week, and answer the associated reading questions.
...
If you're still struggling with some of the content after working through the above materials, head over to the
[Extra Resources](extra-resources.html) page and take a look at the embedded tutorial.
...
# knit_child(paste0(partDir0, "reading-placeholder.Rmd"), quiet = TRUE) %>% cat(sep = "\n")
# knit_child(paste0(partDir0, "home-placeholder.Rmd"), quiet = TRUE) %>% cat(sep = "\n")
# knit_child(paste0(partDir0, "class-placeholder.Rmd"), quiet = TRUE) %>% cat(sep = "\n")
